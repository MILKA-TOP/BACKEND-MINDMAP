...
import mmap.features.maps.models.request.*
import java.util.*
import kotlin.coroutines.coroutineContext

class MapsEditUpdateController(private val call: ApplicationCall) {
    suspend fun update(userId: String, mapId: String) {
        val userIdInt = userId.toInt()
        val mapIdInt = mapId.toInt()
        val updatedMap = call.receive<MapsUpdateRequestParams>()

        val mapAdminId = Maps.selectAdminId(mapIdInt)

        if (mapAdminId == userIdInt) {

            val scope = CoroutineScope(coroutineContext)

            val savedMap = Maps.fetchEditSummary(mapIdInt)
            val nodesIdsMap = createUuidIds(updatedMap.nodes.insert) { it.nodeId }.toMutableMap()
            val testsIdsMap = createUuidIds(updatedMap.tests.insert) { it.testId }.toMutableMap()
            val questionsIdsMap = createUuidIds(updatedMap.questions.insert) { it.questionId }.toMutableMap()
            val answersIdsMap = createUuidIds(updatedMap.answers.insert) { it.answerId }.toMutableMap()
            ...
            val nodesDTOAsync = scope.async {
                updatedMap.nodes.toNodesDTO(
                    savedMap = savedMap,
                    nodesIdsMap = nodesIdsMap,
                )
            }
            val testsDTOAsync = scope.async {
                updatedMap.tests.toTestsDTO(
                    savedMap = savedMap,
                    nodesIdsMap = nodesIdsMap,
            ...
            val questionsDTOAsync = scope.async {
                updatedMap.questions.toQuestionsDTO(
                    savedMap = savedMap,
            ...
            val answersDTOAsync = scope.async {
                updatedMap.answers.toAnswersDTO(
                    savedMap = savedMap,
            ...
            call.respond(AccessDenied, "You can't update this map")
        }
    }
